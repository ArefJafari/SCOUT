...
# with modifications for the SCOUT-SWA architecture by [Your Name or Org].
...
def slide_update(
    ...
    """
    Custom update function for sliding window attention that only keeps the sliding window size of KV cache.
    """
    if layer_idx == 0:
        self._seen_tokens += key_states.shape[-2]
    ...
    # Concatenate new key/value states with existing cache
    ...
    # Keep only the last sliding_window_size tokens
        ...
        if self.attn == "scout":
            ...
            if q_len > 1:
                ...
                if past_key_value is not None:
                    ...
                    selected_key_states, selected_key_states = past_key_value.update(selected_key_states, selected_value_states, self.layer_idx*2+1, cache_kwargs)
            ...
            # decoding, fall back to standard attention
                ...
                if past_key_value is not None:
                    # sin and cos are specific to RoPE models; cache_position needed for the static cache
                    cache_kwargs = {"sin": sin, "cos": cos, "cache_position": cache_position}
                    key_states, value_states = past_key_value.maybe_update(key_states, value_states, self.layer_idx*2+1, self.selection_window_size, cache_kwargs)
            ...
            if past_key_value is not None:
                # sin and cos are specific to RoPE models; cache_position needed for the static cache
                cache_kwargs = {"sin": sin, "cos": cos, "cache_position": cache_position}
                # Use custom slide_update for sliding window attention to limit cache size
                key_states, value_states = past_key_value.slide_update(key_states, value_states, self.layer_idx*2, self.sliding_window_size, cache_kwargs)
...
class ScoutSWAModel(ScoutSWAPreTrainedModel):
    config_class = ScoutSWAConfig
    """
    Transformer decoder consisting of *config.num_hidden_layers* layers. Each layer is a [`ScoutSWADecoderLayer`]
    ...
    def forward(
        ...
        **flash_attn_kwargs: Unpack[FlashAttentionKwargs],
        ...
        # always provide flash_attention mask for SWA
        fa_causal_mask = self._update_fa_causal_mask(
        ...
        if attention_mask is not None and past_key_values is not None:
            ...
            if is_padding_right:
                raise ValueError(
                    ...
                    " this may lead to unexpected behaviour for Flash Attention version of ScoutSWA. Make sure to "
...
class ScoutSWAForCausalLM(ScoutSWAPreTrainedModel, GenerationMixin):
    ...
    def forward(
        ...
        **kwargs: Unpack[KwargsForCausalLM],
        ...
        ```python
        >>> from transformers import AutoTokenizer, ScoutSWAForCausalLM

        >>> model = ScoutSWAForCausalLM.from_pretrained("meta-ScoutSWA/ScoutSWA-2-7b-hf")
        >>> tokenizer = AutoTokenizer.from_pretrained("meta-ScoutSWA/ScoutSWA-2-7b-hf")

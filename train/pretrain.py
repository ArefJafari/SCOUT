...
# Additional modifications made by [Your Name].
...
from data.packed_dataset import CombinedDataset, PackedDataset
from train.utils import chunked_cross_entropy
...
def main(args):
    ...
    wandb_logger = WandbLogger(project="scout", name=args.exp_name+"_"+str(time.time()), id=args.exp_name+"_"+str(time.time()), save_dir=args.wandb_dir, dir=args.wandb_dir, version=args.exp_name, group=args.exp_group)
    deepspeed_config = {
    ...
    if os.path.exists(args.out_dir):
        ...
        if fabric.global_rank == 0:
            ...
            target_model_save_dir = os.path.join(args.out_dir, 'transformer')
            target_bash_scripts_save_dir = os.path.join(args.out_dir, 'bash_scripts')
            os.makedirs(target_model_save_dir)
    ...
    with fabric.init_module(empty_init=True):
        ...
        model.apply(partial(model._init_weights))
    ...
    state = {"model": model, "optimizer_state": optimizer, "hparams": args.hparams, "iter_num": 0, "step_count": 0}
    ...
    train(args, fabric, state, train_dataloader, val_dataloader, args.resume)
    if fabric.global_rank == 0:
...
def train(args, fabric, state, train_dataloader, val_dataloader, resume):
    model = state["model"]
    optimizer = state["optimizer_state"]
    total_lengths = 0
    total_t0 = time.perf_counter()    
    max_tokens_per_device = args.max_tokens // fabric.world_size
    tokens_per_iter = args.micro_batch_size * args.block_size
    ...
    for train_data in train_dataloader:
        tokens += args.block_size * args.micro_batch_size
        if resume:
        ...
        input_ids = train_data[:, 0 : args.block_size].contiguous()
        targets = train_data[:, 1 : args.block_size + 1].contiguous()
        ...
        if fabric.global_rank == 0 and state["iter_num"] % 10 == 0:
            total_tokens = args.block_size * state["iter_num"] * args.micro_batch_size * fabric.world_size / 1e9
            fabric.print(
            ...
            fabric.log_dict({'metric/loss': loss.item(), 'metric/step': state['step_count']})       
...
def create_dataloaders(
    ...
    block_size: int,
    fabric,
    train_data_dir: Path = Path("/work/nicole/dataset/fineweb-edu/15B/prepared"),
...
if __name__ == "__main__":
    ...
    group.add_argument('--block_size', type=int, default=4096, help='train sequence length')

    args = parser.parse_args()
    name = args.train_config +"_" + args.exp_name
    args.out_dir = args.output_root + name
    args.wandb_dir = args.output_root + '/wandb/' + name
  
    train_data_config = [("", 1.0)]
    nodes = 1
    args.nodes = nodes
